...
const chatSocket = () => {
    ...
    useEffect(async () => {
        ...
        setHistory(response.data[0].user)
        setRoomSelected(response.data[0].room);
    ...
    const joinRoomId = (index) => {
        console.log("activé");
        socket.emit('salle rejoint', roomSelected[index].id)
    }
                ...
                <div>
                    <div className="ml-16 flex-wrap w-full h-80 border border-sky-500 bg-white">
                        ...
                        {roomSelected.map((room, index) => (
                            <div key={index}>
                                <button  onClick={joinRoomId(index)}>{room.name}</button>
                            </div>
                        ))}
                    </div>
                </div>
